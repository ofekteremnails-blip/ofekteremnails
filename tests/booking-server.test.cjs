...
context.SpreadsheetApp={flush(){}}; context.getSheet=()=>sheet; context.saveClient=()=>{};
...
const status=params=>JSON.parse(context.doGet({parameter:{action:'bookingStatus',id:'admin-override',date:data.date,time:data.time,duration:60,...params}}).text);
assert.equal(status({}).saved,true);
assert.equal(status({id:'missing'}).saved,false);
assert.equal(status({time:'18:00'}).saved,false);
assert(!JSON.stringify(status({})).includes('clientPhone'));
console.log('PASS read-only booking receipt checks exact ID/slot without client details');
