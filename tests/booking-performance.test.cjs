...
(async () => {
  ...
  const request=b.scripts.at(-1); request.onerror(); b.reply(request,{success:true});
  ...
  c.saveToSheetsWithConflictCheck({...appt,id:'other'},r=>results.push(r)); b.timeout(30000); assert.equal(results.at(-1).error,'timeout');
  c.saveToSheetsWithConflictCheck({...appt},r=>results.push(r)); b.reply(b.scripts.at(-1),{}); assert.equal(results.at(-1).success,false);
